...
SKETCHYBAR_TIMER_TAGS="${SKETCHYBAR_TIMER_TAGS:-sales,marketing,delivery,hr,finance,other}"
SKETCHYBAR_TIMER_RECENT_COUNT="${SKETCHYBAR_TIMER_RECENT_COUNT:-4}"
...
_render_popup() {
  local status_json active duration detail
  ...
  if [ "$active" = "true" ]; then
    ...
    sketchybar --set "$TIMER_ITEM.popup.$slot" drawing=on \
      label="■  Stop timer" label.color="$accent_color"
    sketchybar --set "$TIMER_ITEM.popup.$slot" \
      click_script="$MAKARON_PATH/bin/makaron-timer stop >/dev/null 2>&1; sketchybar --set $TIMER_ITEM popup.drawing=off; sketchybar --trigger timer_refresh"
    slot=$((slot + 1))
  else
    # --- Inactive: Tag list ---
    IFS=',' read -ra _tags <<< "$SKETCHYBAR_TIMER_TAGS"
    for _tag in "${_tags[@]}"; do
      ...
      sketchybar --set "$TIMER_ITEM.popup.$slot" drawing=on \
        label="  ▶  $_tag" label.color="$accent_color"
      sketchybar --set "$TIMER_ITEM.popup.$slot" \
        click_script="$MAKARON_PATH/bin/makaron-timer start $_tag >/dev/null 2>&1; sketchybar --set $TIMER_ITEM popup.drawing=off; sketchybar --trigger timer_refresh"
      slot=$((slot + 1))
    done
  fi
  ...
  local recent_limit="$SKETCHYBAR_TIMER_RECENT_COUNT"
  ...
  if [ "$recent_count" -eq 0 ]; then
    ...
    local i=0
    while [ "$i" -lt "$recent_count" ] && [ "$i" -lt "$recent_limit" ] && [ "$slot" -le "$POPUP_SLOTS" ]; do
      ...
      sketchybar --set "$TIMER_ITEM.popup.$slot" drawing=on label="$row" \
        label.color="$label_color" click_script=""
      slot=$((slot + 1))
      i=$((i + 1))
    done
  fi
  ...
  # --- Help section ---
  if [ "$slot" -le "$POPUP_SLOTS" ]; then
    sketchybar --set "$TIMER_ITEM.popup.$slot" drawing=on label="── CLI ──" \
      label.color="$dim_color" click_script=""
    slot=$((slot + 1))
  fi
  if [ "$slot" -le "$POPUP_SLOTS" ]; then
    sketchybar --set "$TIMER_ITEM.popup.$slot" drawing=on \
      label="  timew summary today" label.color="$dim_color" click_script=""
    slot=$((slot + 1))
  fi
  if [ "$slot" -le "$POPUP_SLOTS" ]; then
    sketchybar --set "$TIMER_ITEM.popup.$slot" drawing=on \
      label="  timew summary :week" label.color="$dim_color" click_script=""
    slot=$((slot + 1))
  fi
  if [ "$slot" -le "$POPUP_SLOTS" ]; then
    sketchybar --set "$TIMER_ITEM.popup.$slot" drawing=on \
      label="  timew export today | jq" label.color="$dim_color" click_script=""
    slot=$((slot + 1))
  fi
